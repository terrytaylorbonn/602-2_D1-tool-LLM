# cursor_10_llm_normalize.py
# ### 10 llm: This file starts from cursor_09_mongo_api.py and adds text normalization with an LLM endpoint.
#
# 3.10 -- Mongo read API + LLM text normalization endpoint
#
# Purpose:
...
#   uvicorn cursor_10_llm_normalize:app --reload --port 8092
#
# Example:
...
# ### 10 llm: Create API app instance for Mongo read + LLM normalization routes.
app = FastAPI(title="Cursor 10 LLM Normalize API", version="0.1.0")
...
def llm_normalize_text(text: str) -> Dict[str, Any]:
    ...
    # Defensive coercion for consistent API output.
...
def normalize_text(payload: NormalizeTextRequest) -> Dict[str, Any]:
    # ### 10 llm: LLM-first normalization endpoint with deterministic fallback.
    ...
    if llm_client is None:
        return {"ok": True, "used_llm": False, "normalized": fallback}

    try:
        normalized = llm_normalize_text(text)
        return {"ok": True, "used_llm": True, "normalized": normalized}
    except Exception as e:
        return {
            "ok": True,
            "used_llm": False,
